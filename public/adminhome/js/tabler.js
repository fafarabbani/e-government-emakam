...
const sessionActiveLink = sessionStorage.getItem('activeLink');
const initialActiveLink = document.querySelector(`.navbar-nav li[href="${sessionActiveLink}"]`);

if (initialActiveLink) {
    initialActiveLink.classList.add('active');
}

function handleActiveLink(clickedLink) {
    // Hapus class "active" dari semua link
    const activeLinks = document.querySelectorAll('.navbar-nav .active');
    activeLinks.forEach(link => link.classList.remove('active'));

    // Tambahkan class "active" pada li yang memuat link yang ditekan
    clickedLink.parentElement.classList.add('active');

    // Simpan link aktif ke sessionStorage
    sessionStorage.setItem('activeLink', clickedLink.href);
}
